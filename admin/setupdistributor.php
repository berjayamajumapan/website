...
if(isset($_POST['simpan'])){
	...
	if($_POST['no']=="AUTO"){ //tambah
	   $no=ambilnomor("ID","t_distributor","","000");
		$field[1][1]="ID";
		$field[1][2]="'".$no."'";
		$field[2][1]="Wilayah";
		$field[2][2]="'".$_POST['wilayah']."'";
		$field[3][1]="Alamat";
		$field[3][2]="'".$_POST['alamat']."'";
		$field[4][1]="Telp";
		$field[4][2]="'".$_POST['telp']."'";
		...
		$field[6][1]="";
		$field[6][2]="";
		generate_query("insert",$field,"t_distributor","","");
	}
	else{ //edit
		if($gambar=="")
		...
		$field[6][1]="";
		$field[6][2]="";
		generate_query("update",$field,"t_distributor","where ID='".$_POST['no']."'","");
		if(isset($_GET['no'])){
			$_GET['no']="";
		}
	}
...
if(isset($_POST['pub'])){
	$field[1][1]="Publish";
	$field[1][2]="'0'";
	generate_query("update",$field,"t_distributor","","");
	while(list($key, $value)=each($_POST['publish'])){
		$field[1][1]="Publish";
		$field[1][2]="'1'";
		generate_query("update",$field,"t_distributor","where ID='".$_GET['no']."'","");
	}
}

if(isset($_GET['hapus'])){
	if($_GET['no']!=""){
		generate_query("delete","","t_distributor","where ID='".$_GET['no']."'","");
	}
	if(isset($_GET['no'])){
		$_GET['no']="";
	}
...
if(isset($_GET['edit'])){
	if($_GET['no']!=""){
		$result1= mysql_query("select * from t_distributor where ID='".$_GET['no']."'");
		$row1=mysql_fetch_row($result1);
	}
}

$result = mysql_query("select * from t_distributor order by ID");
				...
				<tr>
					<td>
						<table class="tekshitamkecil" cellspacing="0">
							<form action="setupdistributor.php" method="post" enctype="multipart/form-data">
							<tr>
								<td width="140">
									ID 
								</td>
							...
							<tr>
								...
								<td class="rapet">	
									<input type="text" name="wilayah" size="50" class="tekshitamkecil" value="<?php
									if(isset($_GET['edit'])){
										echo $row1[1];
									}
									?>">
						...
						<table  bgcolor="#333333" cellspacing="1" border="0" width="100%">
							...
							<tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
								...
								<td>
									<a href="setupdistributor.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
								</td>
								<td>
									<a href="setupdistributor.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
								</td>
								<td>
