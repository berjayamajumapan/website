...
if(isset($_POST['simpan'])){
        ...
        if($_POST['no']=="AUTO"){ //tambah
                ...
                $field[5][1]="";
                $field[5][2]="";
                generate_query("insert",$field,"tw_gallery","","");
        }
        else{ //edit
                if($gambar=="")
                ...
                $field[5][1]="";
                $field[5][2]="";
                generate_query("update",$field,"tw_gallery","where ID='".$_POST['no']."'","");
                if(isset($_GET['no'])){
                        $_GET['no']="";
                }
        }
...
if(isset($_POST['pub'])){
        $field[1][1]="Publish";
        $field[1][2]="'0'";
        generate_query("update",$field,"tw_gallery","","");
        while(list($key, $value)=each($_POST['publish'])){
                $field[1][1]="Publish";
                $field[1][2]="'1'";
                generate_query("update",$field,"tw_gallery","where ID='".$value."'","");
        }
}

if(isset($_GET['hapus'])){
        if($_GET['no']!=""){
                generate_query("delete","","tw_gallery","where ID='".$_GET['no']."'","");
        }
        if(isset($_GET['no'])){
                $_GET['no']="";
        }
...
if(isset($_GET['edit'])){
        if($_GET['no']!=""){
                $result1= mysql_query("select * from tw_gallery where ID='".$_GET['no']."'");
                $row1=mysql_fetch_row($result1);
        }
}

$result = mysql_query("select * from tw_gallery order by ID desc");
										...
										// ak_wysiwyg_generator();  
                                                ...
                                                <table  bgcolor="#333333" cellspacing="1" border="0" width="80%">
                                                        ...
                                                        <tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
                                                                ...
                                                                <td>
                                                                        <a href="setupgallery.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
                                                                </td>
                                                                <td>
                                                                        <a href="setupgallery.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
                                                                </td>
                                                                <td>
