...
if(isset($_POST['simpan'])){
	if($_POST['no']=="AUTO"){ //tambah
		$no=ambilnomor("IDProduct","tbl_parameter","0","000000000");
		$field[1][1]="ID";
		$field[1][2]="'".$no."'";
		$field[2][1]="Title";
		...
		$field[5][1]="";
		$field[5][2]="";
		generate_query("insert",$field,"tbl_product_service","","");
	}
	else{ //edit
		$field[1][1]="ID";
		$field[1][2]="'".$_POST['no']."'";
		...
		$field[5][1]="";
		$field[5][2]="";
		generate_query("update",$field,"tbl_product_service","where ID='".$_POST['no']."'","");
		if(isset($_GET['no'])){
			$_GET['no']="";
		}
	}
...
if(isset($_POST['pub'])){
	$field[1][1]="Publish";
	$field[1][2]="'0'";
	generate_query("update",$field,"tbl_product_service","","");
	while(list($key, $value)=each($_POST['publish'])){
		$field[1][1]="Publish";
		$field[1][2]="'1'";
		generate_query("update",$field,"tbl_product_service","where ID='".$value."'","");
	}
}

if(isset($_GET['hapus'])){
	if($_GET['no']!=""){
		generate_query("delete","","tbl_product_service","where ID='".$_GET['no']."'","");
	}
	if(isset($_GET['no'])){
		$_GET['no']="";
	}
...
if(isset($_GET['edit'])){
	if($_GET['no']!=""){
		$result1= mysql_query("select * from tbl_product_service where ID='".$_GET['no']."'");
		$row1=mysql_fetch_row($result1);
	}
}

$result = mysql_query("select * from tbl_product_service order by ID");
				...
				<tr>
					<td>
						<table class="tekshitam" cellspacing="0">
							<form action="setupproduct.php" method="post" enctype="multipart/form-data">
							<tr>
								<td>
									ID
								</td>
						...
						<table  bgcolor="#333333" cellspacing="1" border="0" width="90%">
							...
							<tr bgcolor="#FDFDFD" class="tekshitam" valign="top">
								...
								<td align="center">
									<a href="setupproduct.php?edit=1&no=<?php echo $row[0]?>" class="link">Edit</a>
								</td>
								<td align="center">
									<a href="setupproduct.php?hapus=1&no=<?php echo $row[0]?>" class="link">Delete</a>
								</td>
								<td align="center">
									<a href="setupimages.php?no=<?php echo $row[0]?>" class="link">Setup Pictures</a>
								</td>
								<td>
